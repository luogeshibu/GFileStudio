# G File Studio v2.17.0

G File Studio 是用于处理 XML 格式 `.sln.pic.g` / `.g` 文件的 PySide6 桌面工具。本版新增“将馈线名称移动到母线上方”，完全不依赖模型关联字段，只依据主母线与文字的图形结构进行保守识别。
...
勾选后点击“开始基础处理”，程序执行以下规则：

1. 只识别有实际长度的水平 `<Bus>`，不处理 `<BusDis>` 和零长度 Bus；
2. 将上下平行、长度接近且 X 范围高度重叠的 Bus 识别为同一双母线组；
3. 只依据 `<Text>` 的 `ts、x、y、w、h、fs/p_FontHeight` 及局部几何关系识别馈线名称；
4. **不读取、不依赖 `key_name` 和 `keyid`**，未关联模型的原始 G 文件也可处理；
5. 排除纯数字、小数、括号数字、单位、`Y1/Q1/SMART/SMR` 等设备标签和说明文字；
6. 候选必须唯一且明显优于其他候选，否则跳过并记录告警；
7. 单母线按本母线水平居中，双母线按整组水平居中，并放到最上方母线正上方；
8. 只修改目标 `<Text>` 的 `x、y`，文字内容、ID、字体、字号、颜色、母线、设备、连接线和模型字段全部保持不变；
9. 目标位置与其他文字发生明显冲突时逐级上移；仍无法避让时跳过；
10. 重复执行保持幂等，已经在目标位置的文字不再变化。

### 真实文件验证

- `$busbartextmove.sln.pic.g`：识别 3 条有效 Bus、2 个母线组，正确移动 2 个 `ABS-36`，排除数值 `255`；
- `JED-NTH-ABH-22.sln.pic.g`：正确移动 `ABH-22`，排除 `2000.00`、`UPDATED_MEASURMENT`、设备编号及其他远端标题；
- 两份文件均只修改目标 Text 的 `x/y`；
- 自动化测试 116 项全部通过。
...
release\GFileStudio_v2.17.0_Windows_x64.zip
```
