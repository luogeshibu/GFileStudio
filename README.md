...
- 所有 G 文件输入模块新增统一的 `SSH 远程 G 文件（只读）` 输入方式：异常小尺寸图元检测、ID 检查与修复、环网柜处理、基础处理、馈线图合并、图形边距调整、图框添加。
- 默认 SSH：`172.16.21.27:22`，用户 `up8000`，远程目录 `/home/up8000/data/graph/display/sln`。
- 远程文件列表支持搜索、单选/多选/当前结果全选，并显示文件大小和服务器修改时间。
- 支持把选中的一个或多个 G 文件单独下载到用户指定的本地目录。
- 业务处理时先把当前选择下载到模块独立的本地快照目录，再复用现有处理引擎；服务器端严格只读，不提供上传、覆盖、重命名、删除或修改接口。
- 馈线合并远程模式会先下载当前选择，再复用原有“加载/检查 → 查询并导入 → 排序 → 合并”流程。
