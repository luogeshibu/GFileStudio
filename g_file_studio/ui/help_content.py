...
<h3>自定义设备图元</h3>
<p>点击“添加设备图元”可手工增加任意规则；检查业务 G 时发现尚未纳入标准的新 devref，会只提示一次并进入“待确认图元”；用户可逐个选择加入当前标准、不纳入本标准或稍后处理。每条自定义规则可设置范围 ANY/SMART/NORMAL、设备角色、XML 元素、标准 devref，以及用于定位业务 G 中当前图元的匹配属性：devref、XML 元素、p_NameString 或 key_name。检查时会比较图元类型/变体、devref 与已学习的几何参数；任何差异只记录为告警，不执行替换。</p>
<h3>标准检查</h3>
<p>“检查图元标准”是完全只读模式：源 G 文件不会被修改、覆盖或复制到 final 目录。程序比较图元类型/变体、devref、w/h、AlignCenter 与 pin 锚点等标准信息，并生成 CSV/HTML 图元标准检查报告；发现差异时界面会明确告警。</p>
<h3>与图元版本升级的边界</h3>
