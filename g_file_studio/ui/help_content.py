...
<p>“修改含 SMART 的环网柜外框颜色”只查找 rect 框内 ts=SMART 的 Text，并修改对应 rect 的 lc/lcc。SMART 字体本身、不含 SMART 的环网柜外框和框外 SMART 文字均保持不变。</p>
<p>启用“移动环网柜红色状态点（channel_status）”后，程序只处理带 BusDis 环网柜内 devref 指向 channel_status 的 &lt;Status&gt;。可选择框内四个角或四条边的中点，并设置距边内边距。该操作只移动红色状态点，不移动环网柜、母线、设备、标题、连接线或画布。</p>
<p><b>环网柜名称与柜型识别：</b>直接解析 G 文件，不使用 OCR。柜名可多选“上方/下方/左侧/右侧”搜索；所选方向为硬约束，未勾选方向绝不参与兜底。每个选中方向先独立寻找紧贴柜框的最近文字组，允许文字包围盒与柜框边缘轻微重叠；多个方向都找到时只在已选方向之间比较真实边距。单候选直接使用，最近组存在多个候选时才优先绿色文字；柜内 Load_Breaker_Switch 计为 L，Circuit_Breaker 计为 T，输出如 2L1T、3L1T。勾选“SMART 参与柜型编码”后追加 0S/1S，例如 3L1T1S。Y/Q/SMART 文字仅用于辅助校验。识别结果写入日志，并同时导出 .rmu.csv 与 .rmu.html；HTML 按置信度和重复柜名/ID 着色。</p>
<h3>馈线名称定位（复选框）</h3>
...
<h3>主母线合并</h3>
<ul>
<li>启用“主网母线处理”后先选择单母线或双母线。单母线只检查 Y 最小的最高有效水平 &lt;Bus&gt;；双母线检查最高母线和同方向下方长度大致相同的第二条有效水平 &lt;Bus&gt;。XML 属性 w&lt;10 的 Bus 完全忽略。只有被选中的主母线必须有非空 keyid；不同 keyid 永远不会互相连接。同一 keyid 必须在馈线排序中连续，并且合并后必须处在同一水平线上，否则拒绝执行。文件名只作人工提醒，不作为硬性拦截条件。</li>
...
<h3>推荐流程</h3>
<ol>
<li>先在“ID 检查与修复”中确认元素 ID 规则并按需检查/修复重复 ID，再在“基础处理”中完成通用规则、环网柜、图元升级和颜色处理。</li>
