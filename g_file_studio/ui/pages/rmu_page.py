...
from g_file_studio.models import BasicOutputConflictAction, BasicSettings, RmuAction, RmuStatusPosition
...
class RmuPage(BasePage):
    ...
    仅承载原基础处理中的“环网柜图元处理”和“环网柜名称与柜型识别”。
    ...
    def __init__(self, user_settings: UserSettingsService, parent=None) -> None:
        ...
        self.layout.addWidget(
            InfoBanner(
                ...
                "channel_status 状态点、带 Bus 外框处理，以及柜名/柜型识别算法均保持原逻辑不变。"
            )
        )
    ...
    def _build_rmu_identification_options(self) -> None:
        ...
        description = QLabel(
            ...
            "柜型优先按 Y1/Y2/... 与 Q1/Q2/... 名称计数，名称无法判断时才回退到设备 devref。SMART 单独输出一列。"
        )
        description.setWordWrap(True)
        description.setObjectName("mutedText")
        layout.addWidget(description)
